...
def handle_product_discovery(conn, table_name, column_names, column_values_dict):
    while True:
        ...
        conjunctions = ['give', 'me', 'the', 'and', 'with', 'for', 'of', 'i', 'need', 'want', 'deatils', 'informations',
            ...
            'supply', 'offer', 'generate', 'issue', 'uncover', 'reveal', 'expose', 'disclose', 'unearth','tell','me'
            'available'
        ]
        ...
        for word in words:
            ...
            if word in map(str.lower, column_values_dict):
                values.append(word)
                print("hi")
            # if any(word in value_list.lower() for value_list in column_values_dict.values()):
            #     values.append(word)
        ...
        if values and not conditions and not columns :
                for val in values:
                    found = False
                    for col_name, col_values in column_values_dict.items():
                        if val.lower() in map(str.lower, col_values):
                            found = True
                            if not conditions:
                                rows = fetch_rows_for_values(table_name, conn, col_name, val)
                                if rows:
                                    print(f"Rows where {col_name} = {val}:")
                                    for row in rows:
                                        print(row)
                                else:
                                    print(f"No matching records found for {col_name} = {val}")
                    if not found:
                        print(f"No matching columns found for value '{val}'")
...
if __name__ == "__main__":
    try:
        ...
        new_values = (column_values_dict.values())
        print(new_values)
